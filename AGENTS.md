...
- Tablet **only** writes under `jobs/pending/spec-<id>/` on the `Codex-jobs` sidecar branch. Never on `main`. Never deletes (append-only from tablet side).
- `05-approved` is created **only** by the tablet. The desktop watcher refuses to implement a spec without it. This is the phase gate the README calls "Codex physically cannot skip ahead."
...
Sync Down sequence (`SyncService.syncDown`, see IMPLEMENTATION.md §4.6 D-13): `fetch origin` → fast-forward local `main` onto `origin/main` → ensure local `Codex-jobs` exists → fast-forward `Codex-jobs` to `origin/Codex-jobs` → merge `main` into `Codex-jobs`. The `main` update is mandatory; without it the sidecar misses newly pushed source files.
